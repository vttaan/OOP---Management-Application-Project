...
Salary_Control::Salary_Control(QObject *parent) : QObject(parent)
{
...
void Salary_Control::loadData(int month, int year)
{
    ...
    SalaryData summary = model->getSalarySummary(month, year);
    QMap<QString, int> normalDays = model->getNormalDaysData(month, year);
    QMap<QString, int> holidayDays = model->getHolidayDaysData(month, year);
    QString baseSalary = model->getBaseSalary();
