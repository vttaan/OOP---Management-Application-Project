...
*   **Functions:**
    ...
    *   `handleGenSchedule()`: Runs the optimizer as a dry-run, presents staffing impact and warnings, then adds an accepted proposal to the in-memory manager draft without writing SQLite.
    *   Manager draft actions support undo/clear, week-scoped validation, grouped before/after review, and a single atomic publish.
...
*   **Functions:**
    ...
    *   `previewGeneratedSchedule(...)`: Runs `Optimizer::solve` and returns an `AutoSchedulePreview` without changing `SHIFT` rows.
    *   `generateSchedule()`: Legacy direct-generation entry point implemented through preview plus atomic application.
...
*   **`Schedule_View` / `ViewSchedule_View`**: Interfaces for managers to create schedules and for all users to view them. `Schedule_View.cpp` owns shared construction and feedback, `Schedule_View_Staff.cpp` owns staff registration and status-grid behavior, `Schedule_View_Manager.cpp` owns manager grids, summaries, and missing-staff rendering, and `Schedule_View_Manager_ShiftDetail.cpp` owns the complete `Chi tiết ca làm` drawer/dialog workflow and its block-scoped employee queue. Calendar weeks always run Monday through Sunday. The configurable registration/open day controls when staff registration and the carry-forward process open, but never changes the week boundary. Manager arrangement, staff registration, carry-forward, and automatic generation target the next Monday-Sunday week; schedule viewing defaults to the current Monday-Sunday week. `Schedule_View` supports the existing 15x7 hourly registration grid and a `PROFILES.isFixed`-selected 3x7 `FullTimeSchedule` grid. Both grids persist pending registrations in `SHIFT`; approved rows are locked, and `Xóa hết` clears only the editable pending selection before `Lưu / Xác nhận` persists it. Managers approve or decline pending rows through the shared assignment workflow. `ManagerEmployeeChooser_Dialog` provides the manager's reusable multi-select staff search, role filter, ID/name sort, eligibility display, and keyboard-only time editing for flexible staff. Manager profiles are excluded entirely; fixed-salary staff use locked canonical block times. Its confirmed block-scoped queue is converted to batch draft changes by `Schedule_Control`. A shared view-owned grid delegate renders both staff layouts with the same four primary states: `Thiếu`, `Đủ`, `Chờ duyệt`, and `Đã duyệt`; full-time staffing states are derived per role from approved `SHIFT` rows. The part-time view receives its open/closed state and next configured opening date from `Schedule_Control` and presents a muted read-only staffing preview while closed. Both staff grids preserve the configured seven-day sequence supplied by `Schedule_Control` and `Config`. *Controllers: `Schedule_Control`, `ViewSchedule_Control`.*
